"""Real Chromium checks; never substitute screenshots for the running product."""
import functools
import http.server
import json
import pathlib
...
with sync_playwright() as p:
    ...
    for label, width, height in [('desktop',1536,1024),('mobile',390,844)]:
        ...
        try:
            page.goto('http://127.0.0.1:8766/preview/concepts/index.html#9701')
            ...
            for key, position in [('organise',.95),('connect',2.15),('rehearse',3.85),('finish',5)]:
                page.evaluate('(p)=>{const h=document.getElementById("projectScroll");h.scrollTop=p*h.clientHeight/(matchMedia("(max-width:699px)").matches?1.25:1);}',position)
            ...
            # Back and Forward still operate the portfolio after iframe creation.
            page.go_back()
            ...
            passed(label+'/back-and-cleanup')
            page.go_forward()
            page.wait_for_selector('#scene.opened')
            passed(label+'/forward')
            ...
            passed(label+'/escape-returns-to-exhibition')
            ...
            answer=frame.locator('#practice-shell textarea').first
            answer.fill('a deliberately incomplete answer')
            ...
            assert page.locator('dialog.atlas-is-live').count()==0
            passed(label+'/visible-return-control')
            ...
            page.goto('http://127.0.0.1:8766/preview/concepts/index.html#cuelayer')
            page.wait_for_selector('#scene.opened')
        ...
        context.close()
    # Verify remote embedding under the actual production portfolio origin.
